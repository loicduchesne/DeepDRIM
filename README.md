...
---
### How to use?
All the script maintain the same entry points as the original DeepDRIM, except for the new *PBMC_data_convert.py*. Important thing to note is that the modifications are rudimentary and have not been extensively tested yet.

Note: Adjust the epochs and batch configuration if needed.

#### Example: PBMC-CTL
Using a Jupyter Notebook for GRN Inference using PBMC-CTL.

*PBMC-CTL dataset from:* https://epbmc.immunospot.com/?cookieCheck=true
#### Step 1: Pre-processing PBMC-CTL Dataset
This step is meant to convert PBMC-CTL data files into a format that can be processed when generating inputs. This step is necessary because of the data files not directly integrating to the DeepDRIM workflow.

In notebook or command line run:
```
# Pre-process PBMC-CTL
!python3 DeepDRIM/PBMC_data_convert.py -expr_path data/PBMC-CTL_1000_cells.csv -truth_path data/PBMC-CTL_Imposed_GRN.csv -output_dir out/DeepDRIM/processed
```

#### Step 2: Generating training inputs
This step generates the necessary inputs to run the training for DeepDRIM.
```
# Generate training inputs
!python3 DeepDRIM/generate_input_realdata.py -out_dir out/DeepDRIM/processed -expr_file data/PBMC-CTL_1000_cells.csv -pairs_for_predict_file out/DeepDRIM/processed/PBMC-CTL_training_pairs.txt -geneName_map_file out/DeepDRIM/processed/PBMC-CTL_geneName_map.txt -flag_load_from_h5 False -flag_load_split_batch_pos False
```

#### Step 3: Training
This is the training step for DeepDRIM.
```
# Training
!python3 DeepDRIM/DeepDRIM.py -num_epochs 200 -num_batches 18 -data_path DeepDRIM/data_processed/version11/ -output_dir DeepDRIM/out -cross_validation_fold_divide_file DeepDRIM/cross_validation_fold_divide2.txt
```
